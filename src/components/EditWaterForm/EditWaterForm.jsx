...
import { updateWaterOperation } from "../../redux/water/operations";
...
import css from "./EditWaterForm.module.css";

const EditWaterForm = ({ onClose }) => {
  ...
  useEffect(() => {
    if (currentItem) {
      setValue("time", formatTimeUTC(currentItem.date));
      setValue("amount", currentItem.amount || 250);
    }
  ...
  const onSubmit = async (data) => {
    ...
    const waterData = {
      amount: data.amount,
      date: `${currentItem.date.slice(0, 10)}T${data.time}:00`,
    };
    try {
      await dispatch(
        updateWaterOperation({ id: currentItem.id, data: waterData })
      ).unwrap();
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className={css.form}>
      <p className={css.amountSubtitle}>{t("correctData")}</p>
